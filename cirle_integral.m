function answer=cirle_integral(up,down,left,right)
%%
...
answer=(cylinder(x+1i*down,fun(x,down))+cylinder(right+y*1i,fun(right,y))...
    +cylinder(fliplr(x)+up*1i,fun(fliplr(x),up)) +cylinder(left+(fliplr(y))*1i,fun(left,fliplr(y))))./(2*pi*1i);
